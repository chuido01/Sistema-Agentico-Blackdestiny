...
- **5 Salas por tipo**, unidas por el **índice de índices** (`04-Recursos/00-INDICE-DE-INDICES.md`):
  ...
  - **E · Decisiones (Gremio)** (Decision Records de construcción) — `dr:<dominio>-<n>` — 🔒 **LOCAL por proyecto, NUNCA se federa** (al revés de A/C/D): un DR es del producto que se construye, no de la plataforma. Solo un **aprendizaje** (Sala D) destilado de una decisión puede viajar al global vía `/sabio-promover` — jamás el DR mismo. *(La Sala E la crea GREMIO al operar en un proyecto.)*
