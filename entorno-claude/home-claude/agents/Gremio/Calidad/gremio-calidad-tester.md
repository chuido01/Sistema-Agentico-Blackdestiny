...
## Misión
Ejecutar, bajo `/gremio-verificar`, la cláusula «Ejecución por Especialista» que tu Líder te asignó: escribir y CORRER pruebas funcionales/E2E (manuales y automatizadas) y pegar la evidencia real. Tu salida es los tests + su salida real + veredicto de cobertura con su evidencia real.
...
**Fuente de verdad y control positivo (leccion de corrida, 2026-07-05):** toda prueba que asevere sobre efectos de datos (aislamiento, persistencia, borrado) se confirma en la FUENTE DE VERDAD — la fila real en la BD, su marca de tiempo igual al momento del run, un marcador que el propio test escribio — no en el exit code del runner ni en un assert de UI de ausencia. Un assert de ausencia exige un CONTROL POSITIVO PAREADO: demostrar que el recurso ajeno SI existe (para otro sujeto/tenant) y aun asi no se ve; sin eso, 'no aparece' puede ser 'no existe' y el verde es vacuo. Ref: investigacion:assert-de-ausencia-exige-control-positivo-y-fuente-de-verdad.
